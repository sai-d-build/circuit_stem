# **Circuit STEM: The Complete Core Logic Refactoring Guide**

**Version:** 2.0
**Author:** Me
**Status:** In Progress (Consolidated from multiple documents)
...
## **Phase 2: In-Place Behavior Standardization**
...
## **Phase 3: Gradual Strangulation & Robust Implementation**

**🎯 Goal:** To repeat the "Adapter" pattern for all other features, systematically replacing the old logic until the `GameEngineNotifierV2` is just an empty shell that delegates all logic to a robust, testable, and maintainable system of Actions, Use Cases, and Middleware.

**🤔 Why this phase is important:** This is the main part of the refactoring. We are methodically and safely migrating the entire application's logic, one piece at a time, reducing risk at every step while simultaneously building a production-grade architecture.

---

### **Current Status & Progress (As of 2025-08-25)**

#### **✅ Excellent Progress Made**
*   **Clean Architecture Foundation**: Successfully established an action-based system with clear separation of concerns.
*   **Provider Architecture**: Proper Riverpod setup with dependency injection is in place.
*   **UI Decoupling**: The `GameCanvas` now dispatches actions instead of making direct method calls.
*   **Code Stability**: All analysis errors have been resolved, and consistent naming conventions have been established.
*   **Completed UI Interactions**:
    *   Create Component from Palette (`CreateComponentFromTemplateAction`)
    *   Move Existing Component (`MoveComponentAction`)
    *   Tap Interaction (`TapComponentAction`)

#### **⚠️ Pending Work & Areas for Robustness Enhancement**
*   **Refactor Power Simulation:** The logic needs to be moved into a dedicated `SimulatePowerFlowUseCase`. The current `PowerSimulationService` is just a placeholder.
*   **Refactor Goal Checking:** The logic needs to be moved into a dedicated `CheckWinConditionUseCase`. The current `GoalCheckingService` is a placeholder.
*   **Convert Direct State Mutations:** Methods like `restartLevel`, `updateComponent`, and `selectPaletteComponent` in `GameEngineNotifier` still modify state directly and must be converted to the action-based use case pattern.
*   **Implement Robustness Enhancements:** The current actions and use cases are simple. They need to be enhanced with the advanced validation, error handling, and middleware patterns outlined below to make the system truly production-ready.

---

### **Enhanced Architecture & Implementation Strategy**

This section details the robust architectural patterns that should be implemented for all new and refactored features in this phase.

#### **1. Enhanced Action System (`ComponentAction`)**

**Issue:** Current actions are basic data containers.
**Enhancement:** Actions should be robust classes with built-in validation and metadata.
...
abstract class ComponentAction extends Equatable {
  ...
  // All actions must be able to validate themselves against the current state.
  Result<void> validate(GameEngineState state) {
    return const Success(null);
  }
}

// Example: A more robust Create action
class CreateComponentFromTemplateAction extends ComponentAction {
  ...
  Result<void> validate(GameEngineState state) {
    ...
    if (state.grid.componentAt(row, col) != null) {
...
#### **2. Robust Error Handling (`Result<T>` Pattern)**

**Issue:** Errors can be handled inconsistently, sometimes with exceptions, sometimes with nulls.
**Enhancement:** Use a `Result<T>` sealed class (`Success` or `Failure`) for all operations that can fail. This enforces explicit error handling and eliminates runtime exceptions from business logic.
...
#### **3. Enhanced Use Case Pattern (`UseCase`)**

**Issue:** Use cases have no standard structure.
**Enhancement:** Create a `UseCase` base class that standardizes execution and validation, using the `Result<T>` pattern.
...
abstract class UseCase<TAction extends ComponentAction, TResult> {
  ...
  // Public execute method handles validation and error catching
  ...
  Result<void> validate(GameEngineState state, TAction action) {
    return action.validate(state); // By default, delegate to the action's own validation
  }
  
  // Subclasses must implement the core logic here
  TResult executeInternal(GameEngineState state, TAction action);
}
```

#### **4. Enhanced Game Engine with Middleware**

**Issue:** The `GameEngineNotifier` has to manually call logging, validation, etc.
**Enhancement:** Implement a middleware pipeline. The notifier's only job is to pass an action through the pipeline and then to the correct use case.
...
class GameEngineNotifier extends StateNotifier<GameEngineState> {
  ...
  Future<void> executeAction(ComponentAction action) async {
      // 1. Apply middleware before action
      ...
      // 2. Execute the appropriate use case
      final result = await _executeUseCase(processedAction);
      
      GameEngineState newState;
      if (result.isSuccess) {
        newState = result.data!;
        // ... run simulation, check win condition, etc. ...
      } else {
        // Handle failure
        return;
      }

      // 3. Apply middleware after action
      for (final middleware in _middleware.reversed) {
        newState = await middleware.afterAction(state, newState, processedAction);
      }
...
**Issue:** Logic like logging, validation, and undo/redo is scattered.
**Enhancement:** Centralize these concerns in self-contained middleware classes.
...
#### **6. Advanced State Management & Testing**

*   **State Validation:** The `GameEngineState` should have methods like `isValidState()` to check for internal consistency (e.g., no duplicate component IDs).
*   **Immutability:** Use `freezed` for all state objects to guarantee immutability.
*   **Testing Strategy:**
    *   **Use Case Tests:** Test each use case in isolation, mocking its dependencies.
    *   **State Validation Tests:** Write tests that ensure invalid states cannot be created.
    *   **Power Simulation Tests:** Write dedicated tests for the simulation service with complex grid layouts.
    *   **Undo/Redo Tests:** Verify that the state history is managed correctly.
...
## **Appendix: Legacy Approaches & Discarded Plans**
...
### **Legacy Plan: The Adapter & Bridge (First Integration)**
...
**🤔 Why this phase was important:** This was our first step in "strangling" the old system. We were building a "bridge" from the old world to the new. By doing this for one small feature (toggling a switch), we could prove the pattern works before applying it to more complex logic like the power simulation.
