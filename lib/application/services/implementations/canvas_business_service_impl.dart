...
class CanvasBusinessServiceImpl implements CanvasBusinessService {
  ...
  Future<ComponentPlacementResult> placeComponent(
    ...
    if (!paletteState.canUseComponent(componentTypeString)) {
      StructuredLogger.warning(
          'Component placement denied - insufficient inventory',
          context: {
            'componentType': componentTypeString,
            'availableInventory': paletteState.inventory,
          });
      return ComponentPlacementResult.failure(
          'No more $componentTypeString components available');
    }
    ...
    final existingComponent = _getComponentAtPosition(
        Offset(snappedPosition.dx.toInt().toDouble(),
            snappedPosition.dy.toInt().toDouble()),
        gameState);

    if (existingComponent != null) {
      StructuredLogger.warning('Component placement denied - position occupied',
          context: {
            'position':
                '${snappedPosition.dx.toInt()}, ${snappedPosition.dy.toInt()}',
    ...
    StructuredLogger.debug('Placing component in game state', context: {
      ...
      'gridPosition':
          '${snappedPosition.dy.toInt()}, ${snappedPosition.dx.toInt()}',
    });

    // Update game state
    paletteStateNotifier.useComponent(componentTypeString); // ignore: cascade_invocations
    paletteStateNotifier.stopPlacingComponent(); // ignore: cascade_invocations
    ...
    return ComponentPlacementResult.success(
        componentType, snappedPosition.dy.toInt(), snappedPosition.dx.toInt());
  }
  ...
  Future<ComponentDropResult> processComponentDrop(
    ...
    final componentType = ComponentType.values.firstWhere(
      (e) =>
          ...
          details.data.componentType.toString().split('.').last,
      orElse: () => ComponentType.wire,
    );
    ...
    try {
      ...
      if (result.isSuccess) {
        ...
        return ComponentDropResult.failure(
            result.errorMessage ?? 'Failed to place component');
      }
    } catch (e) {
      StructuredLogger.error('Component drop error',
          context: {
            'error': e.toString(),
          },
          error: e);
  ...
  bool canAcceptComponentDrop(
    ...
    final config = _getGridConfiguration(gameState);
    final unifiedService = UnifiedCoordinateService();
    final validGridPosition =
        unifiedService.getValidGridPosition(localPosition, config);
    ...
    final existingComponent =
        _getComponentAtPosition(validGridPosition, gameState);
    if (existingComponent != null) {
      StructuredLogger.debug('Drop validation failed - position occupied',
          context: {
            'occupyingComponent': existingComponent.id,
          });
      return false;
    ...
    // Check inventory
    final componentTypeString =
        details.data.componentType.toString().split('.').last;
    final canUse = paletteState.canUseComponent(componentTypeString);

    if (!canUse) {
      StructuredLogger.debug('Drop validation failed - insufficient inventory',
          context: {
            'componentType': componentTypeString,
          });
      return false;
