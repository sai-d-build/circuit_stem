...
class DefaultComponentPlacementService implements ComponentPlacementService {
  ...
  PlacementValidationResult validatePlacement(
    ...
    // Check grid bounds
    final boundsCheck =
        _gridValidationService.validateBounds(row, col, gameState);
    if (!boundsCheck.isValid) {
      return PlacementValidationResult.invalid(
          boundsCheck.reason ?? 'Position out of bounds');
    }

    // Check position availability
    final availabilityCheck =
        _gridValidationService.validateAvailability(row, col, gameState);
    if (!availabilityCheck.isValid) {
      return PlacementValidationResult.invalid(
          availabilityCheck.reason ?? 'Position occupied');
    }
  ...
  Future<PlacementExecutionResult> executeComponentPlacement(
    ...
    try {
      ...
      if (!consumptionResult.isSuccess) {
        return PlacementExecutionResult.failed(consumptionResult.errorMessage ??
