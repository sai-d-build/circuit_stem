...
class PowerSimulationService {
  Grid simulatePowerFlow(Grid grid) {
    Logger.log(
        'PowerSimulationService: Simulating power flow on ${grid.getComponentCount()} components');

    // Create a copy of the grid to modify
    final newComponents = Map<String, ComponentModel>.from(grid.components);

    // Find power sources (batteries)
    final batteries = grid
        .getAllComponents()
  ...
  void _propagatePower(Grid grid, ComponentModel source,
    ...
    for (final connectedId in connections) {
      ...
      if (connectedComponent != null &&
          connectedComponent.state != ComponentState.powered) {
        // Propagate to connected component
        components[connectedId] =
            connectedComponent.copyWith(state: ComponentState.powered);
