...
abstract class UseCase<TAction extends ComponentAction> {
  ...
  Future<Result<GameEngineState>> execute(
      GameEngineState state, TAction action);
}
...
class UseCaseAdapter {
  ...
  Future<Result<GameEngineState>>
      executeUseCase<TAction extends ComponentAction>(
          UseCase<TAction> useCase, TAction action) async {
    return await _orchestrator.executeUseCase(useCase, action);
  }

  /// Execute a use case directly against a provided state (for testing)
  Future<Result<GameEngineState>>
      executeUseCaseWithState<TAction extends ComponentAction>(
