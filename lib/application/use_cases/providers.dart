...
final checkWinConditionUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'check_win_condition_use_case.dart', DateTime.now().toIso8601String());
...
final loadLevelUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'load_level_use_case.dart', DateTime.now().toIso8601String());
  final powerSimulationService = ref.watch(powerSimulationServiceProvider); // ignore: cascade_invocations
  final goalCheckingService = ref.watch(goalCheckingServiceProvider); // ignore: cascade_invocations
  return LoadLevelUseCase(powerSimulationService, goalCheckingService);
});

final moveComponentUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'move_component_use_case.dart', DateTime.now().toIso8601String());
...
final restartLevelUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'restart_level_use_case.dart', DateTime.now().toIso8601String());
...
final rotateComponentUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'rotate_component_use_case.dart', DateTime.now().toIso8601String());
  return const RotateComponentUseCase();
});

final selectPaletteComponentUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated('select_palette_component_use_case.dart',
...
final simulatePowerFlowUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'simulate_power_flow_use_case.dart', DateTime.now().toIso8601String());
  final simulationService = ref.watch(powerSimulationServiceProvider);
...
final tapComponentUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'tap_component_use_case.dart', DateTime.now().toIso8601String());
  final simulationService = ref.watch(powerSimulationServiceProvider);
...
final togglePauseUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'toggle_pause_use_case.dart', DateTime.now().toIso8601String());
  return const TogglePauseUseCase();
});

final undoUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'undo_use_case.dart', DateTime.now().toIso8601String());
  return UndoUseCase();
});

final updateComponentUseCaseProvider = Provider((ref) {
  MigrationTracker.markFileMigrated(
      'update_component_use_case.dart', DateTime.now().toIso8601String());
  final simulationService = ref.watch(powerSimulationServiceProvider);
