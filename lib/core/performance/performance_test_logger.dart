...
class PerformanceTestLogger {
  ...
  static void testSuiteHeader(String suiteName, [String? description]) {
    StructuredLogger.info('$_testPrefix | SUITE_START | $suiteName', context: {
      'description':
          description ?? 'Enterprise-grade performance regression testing',
  ...
  static void testSuiteComplete(String suiteName, {bool success = true}) {
    StructuredLogger.info(
        '$_testPrefix | SUITE_COMPLETE | $suiteName | ${success ? 'SUCCESS' : 'FAILED'}',
        context: {
          'timestamp': DateTime.now().toIso8601String(),
          'separator': _separator
        });
  ...
  static void testResult(String testName, Map<String, dynamic> metrics,
    ...
    StructuredLogger.info(
        '$_testPrefix | TEST_RESULT | $testName | ${passed ? 'PASSED' : 'FAILED'}',
        context: {
          'metrics': metrics,
          'notes': notes,
          'target': target,
  ...
  static void performanceMetric(String operation, double value, String unit,
    ...
    final message =
        '$operation: ${value.toStringAsFixed(2)}$unit${target != null ? ' (Target: $target)' : ''}';

    StructuredLogger.info('$_testPrefix | METRIC | $message', context: context);
  }

  /// Log cache statistics
  static void cacheStats(Map<String, dynamic> stats) {
    StructuredLogger.info('$_testPrefix | CACHE_STATS', context: stats);
  }
  ...
  static void memoryUsage(double currentMB, double growthMB,
    ...
    StructuredLogger.info(
        '$_testPrefix | MEMORY_USAGE | ${currentMB.toStringAsFixed(1)}MB (Growth: ${growthMB.toStringAsFixed(1)}MB)',
        context: {
  ...
  static void deviceCompatibility(
      String tier, List<String> features, double score,
      {Map<String, dynamic>? details}) {
    StructuredLogger.info(
        '$_testPrefix | DEVICE_COMPATIBILITY | Tier: $tier | Score: ${(score * 100).toStringAsFixed(1)}%',
        context: {
          'tier': tier,
          'features': features,
  ...
  static void errorRecovery(String type, double successRate, double stability,
      {Map<String, dynamic>? details}) {
    StructuredLogger.info(
        '$_testPrefix | ERROR_RECOVERY | $type | Success: ${(successRate * 100).toStringAsFixed(1)}% | Stability: ${(stability * 100).toStringAsFixed(1)}%',
        context: {
          'recovery_type': type,
  ...
  static void sustainedPerformance(
      double avgFrameTime, double fps, double deviance,
      {Duration? duration, String? notes}) {
    StructuredLogger.info(
        '$_testPrefix | SUSTAINED_PERFORMANCE | ${avgFrameTime.toStringAsFixed(2)}ms/frame | ${fps.toStringAsFixed(1)} FPS',
        context: {
  ...
  static void warning(String message, {Map<String, dynamic>? context}) {
    StructuredLogger.warning('$_testPrefix | WARNING | $message',
        context: context);
  }

  /// Log test failures
  static void testFailure(String testName, String reason,
      {Map<String, dynamic>? context}) {
    StructuredLogger.error('$_testPrefix | TEST_FAILURE | $testName | $reason',
        context: context);
  }
