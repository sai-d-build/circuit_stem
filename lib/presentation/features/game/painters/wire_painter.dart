...
class WirePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final wire in wires) {
      final startOffset =
          coordinateService.gridToScreen(Offset(wire.startX, wire.startY));
      final endOffset =
          coordinateService.gridToScreen(Offset(wire.endX, wire.endY));

      // Draw glow effect for active wires
      if (wire.isActive) {
        final glowPaint = Paint()
          ..color = circuitColors.glowEffect.withValues(
              alpha: GameConstants.highOpacity) // Use glowEffect color
          ..strokeCap = StrokeCap.round
          ..strokeWidth = GameConstants.wireGlowRadius *
              coordinateService.scale // Wider for glow
          ..maskFilter = MaskFilter.blur(BlurStyle.normal,
              GameConstants.selectionGlowRadius * coordinateService.scale);
        canvas.drawLine(startOffset, endOffset, glowPaint);
      }

      // Draw main wire
      paint.color =
          wire.isActive ? circuitColors.wireActive : circuitColors.wireInactive;
      canvas.drawLine(
        startOffset,
        endOffset,
        paint,
