...
class _LevelSelectScreenState extends ConsumerState<LevelSelectScreen> {
  ...
  Future<void> _loadLevels() async {
    StructuredLogger.info('LevelSelectScreen: Starting level loading',
        context: {
    ...
    try {
      ...
      if (StructuredLogger.debugPresentation) {
        StructuredLogger.presentation('Getting level service from Riverpod',
            context: {
      ...
      StructuredLogger.debug(
          'LevelSelectScreen: Calling levelService.loadAllLevels()',
          context: {
      ...
      StructuredLogger.info('LevelSelectScreen: Levels loaded successfully',
          context: {
      ...
      StructuredLogger.debug(
          'LevelSelectScreen: Setting state with loaded levels',
          context: {
            'levelsCount': levels.length,
            'isLoading': false,
            'hasError': false,
      ...
      setState(() {
        ...
        StructuredLogger.info('✅ STATE UPDATED: Levels successfully loaded',
            context: {
              'levelsCount': levels.length,
      ...
      StructuredLogger.info(
          'LevelSelectScreen: Level loading completed successfully',
          context: {
            'finalLevelsCount': levels.length,
            'screenState': 'ready',
          });
    } catch (e, stackTrace) {
      StructuredLogger.error('LevelSelectScreen: Failed to load levels',
          context: {
            'error': e.toString(),
  ...
  Future<void> _loadCompletedLevels() async {
    ...
    final completedLevels =
        storageService.readData<List<String>>('completed_levels') ?? [];
    setState(() {
  ...
  Widget _buildBody(CircuitColorScheme colors, TextTheme textTheme) {
    ...
    if (_isLoading) {
      ...
      return Center(
          child: CircularProgressIndicator(color: colors.neonPrimary));
    }

    if (_error != null) {
      StructuredLogger.warning('LevelSelectScreen: Showing error view',
          context: {
            'error': _error,
    ...
    if (_levels == null || _levels!.isEmpty) {
      StructuredLogger.warning('LevelSelectScreen: Showing empty view',
          context: {
            'levelsIsNull': _levels == null,
            'levelsLength': _levels?.length ?? 0,
            'reason':
                _levels == null ? 'levels_not_loaded' : 'no_levels_available',
          });
          ...
          Expanded(
            child: GridView.builder(
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                ...
                crossAxisSpacing: 16,
                mainAxisSpacing: 16,
                childAspectRatio: 1, // Adjusted for better card aspect ratio
              ),
              ...
              itemBuilder: (context, index) {
                ...
                return NeonLevelCard(
                  ...
                  onPressed: isUnlocked
                      ? () {
                          StructuredLogger.info(
                              'LevelSelect: Level selected for navigation',
                              context: {
                      ...
                      : () {
                          StructuredLogger.debug(
                              'LevelSelect: Attempted to select locked level',
                              context: {
                                'levelId': level.levelId,
                                'levelTitle': level.metadata.title,
                                'isLocked': true,
  ...
  bool _isLevelLocked(LevelDefinition level) {
    ...
    return !prerequisites
        .every((prereqId) => _completedLevels.contains(prereqId));
  }
}
