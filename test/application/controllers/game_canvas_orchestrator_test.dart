...
void main() {
  ...
  group('GameCanvasOrchestrator', () {
    ...
    test('should handle gesture input correctly', () {
      ...
      final expectedResult = GestureProcessingResult(
        newState: GameCanvasState.initial().copyWith(
          interactionState: InteractionState(mode: GestureMode.idle, selectedComponentId: 'test-component'),
        ),
      ...
      // Assert
      verify(mockInteractionService.processGesture(event, orchestrator.state)).called(1);
      expect(orchestrator.state.interactionState.selectedComponentId, 'test-component');
    });
    ...
    test('should handle component placement side effects', () {
      ...
      // Assert
      verify(mockPlacementService.executeComponentPlacement(placementRequest)).called(1);
    });

    test('should handle viewport update side effects', () {
      // Arrange
      final event = GestureInputEvent.dragUpdate(Offset(100, 100), Offset(10, 10));
      final newViewportState = ViewportState(
        scale: 1.0,
        panOffset: Offset(10, 10),
        canvasSize: Size(800, 600),
        gridConfiguration: GridConfiguration(rows: 10, cols: 10, cellSize: 60),
      );
      final sideEffect = ViewportUpdateSideEffect(newViewportState: newViewportState);
      final expectedResult = GestureProcessingResult(
        newState: GameCanvasState.initial(),
        sideEffects: [sideEffect],
      );
    ...
    test('should maintain state consistency during gesture processing', () {
      ...
      final newState = GameCanvasState.initial().copyWith(
        interactionState: InteractionState(mode: GestureMode.idle, selectedComponentId: 'new-selection'),
      );
      final expectedResult = GestureProcessingResult(
        newState: newState,
        sideEffects: [],
      ...
      // Assert
      expect(orchestrator.state.interactionState.selectedComponentId, 'new-selection');
