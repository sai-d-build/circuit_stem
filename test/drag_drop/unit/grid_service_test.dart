...
void main() {
  group('GridService Coordinate Conversion Tests', () {
    ...
    test('screenToGrid converts screen coordinates to grid coordinates correctly', () {
      ...
      final screenPos = const Offset(30, 30); // Center of (0,0) cell
    ...
    test('gridToScreen converts grid coordinates to screen coordinates correctly', () {
      ...
      final gridPos = const Offset(0.5, 0.5);
    ...
    test('screenToGrid with scale and pan offset', () {
      final scaledConfig = const GridConfiguration(
        rows: 10,
        cols: 10,
        cellSize: 60.0,
        scale: 2.0,
      ...
      final screenPos = const Offset(130, 80);
    ...
    test('snapToGrid snaps to nearest grid cell center', () {
      ...
      final screenPos = const Offset(85, 85); // Should snap to (1,1)
    ...
    test('isInGridBounds correctly validates grid positions', () {
      expect(GridService.isInGridBounds(const Offset(0, 0), testConfig), isTrue);
      expect(GridService.isInGridBounds(const Offset(5, 5), testConfig), isTrue);
      expect(GridService.isInGridBounds(const Offset(9, 9), testConfig), isTrue);
      expect(GridService.isInGridBounds(const Offset(10, 10), testConfig), isFalse);
      expect(GridService.isInGridBounds(const Offset(-1, -1), testConfig), isFalse);
    });

    test('getValidGridPosition returns valid positions within bounds', () {
      final validPos = GridService.getValidGridPosition(const Offset(150, 150), testConfig);
      expect(validPos, isNotNull);
      expect(validPos!.dx, closeTo(2.0, 0.01)); // 150/60 = 2.5, floor to 2
      expect(validPos!.dy, closeTo(2.0, 0.01));

      final invalidPos = GridService.getValidGridPosition(const Offset(-50, -50), testConfig);
