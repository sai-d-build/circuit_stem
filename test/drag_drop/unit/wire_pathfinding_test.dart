...
void main() {
  group('Wire Pathfinding', () {
    test('should calculate Manhattan path between adjacent ports', () {
      ...
      expect(path[0], GridPosition(row: 0, col: 0));
      expect(path[1], GridPosition(row: 0, col: 1));
      expect(path[2], GridPosition(row: 0, col: 2));
    });
    ...
    test('should calculate L-shaped Manhattan path', () {
      ...
      expect(path.length, 5); // start -> horizontal -> corner -> vertical -> end
      expect(path[0], GridPosition(row: 0, col: 0));
      expect(path[1], GridPosition(row: 0, col: 1));
      expect(path[2], GridPosition(row: 0, col: 2));
      expect(path[3], GridPosition(row: 1, col: 2));
      expect(path[4], GridPosition(row: 2, col: 2));
    });
    ...
    test('should handle reverse direction paths', () {
      ...
      expect(path[0], GridPosition(row: 2, col: 2));
      expect(path[1], GridPosition(row: 2, col: 1));
      expect(path[2], GridPosition(row: 2, col: 0));
      expect(path[3], GridPosition(row: 1, col: 0));
      expect(path[4], GridPosition(row: 0, col: 0));
    });
    ...
    test('should handle large grid distances', () {
      ...
      expect(path.last, GridPosition(row: 10, col: 15));
    });
    ...
    test('should maintain connectivity between adjacent path segments', () {
      ...
      for (int i = 0; i < path.length - 1; i++) {
        ...
        expect(isAdjacent, true, reason: 'Positions $current and $next should be adjacent');
      }
    });
  });
  ...
  group('Wire Validation', () {
    ...
    test('should detect wire placement conflicts', () {
      ...
      final wirePath = [
        ...
        GridPosition(row: 1, col: 1), // Conflict here
        GridPosition(row: 1, col: 2),
      ];
...
bool _checkWireConflicts(List<GridPosition> wirePath, Set<GridPosition> occupiedPositions) {
