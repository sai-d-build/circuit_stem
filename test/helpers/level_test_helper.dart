...
class LevelTestHelper {
  ...
  static ComponentModel? findComponentById(GameEngineState state, String id) {
  ...
  static Future<void> dragComponent(WidgetTester tester,
      GameEngineNotifier notifier, String id, int toRow, int toCol) async {
    final comp = findComponentById(notifier.state, id);
    if (comp == null) return;

    final from = componentCenter(comp);
    final to = Offset(
        toCol * cellSize + cellSize / 2, toRow * cellSize + cellSize / 2);
