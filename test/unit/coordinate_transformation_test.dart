  ...
  group('Coordinate Transformation Tests', () {
    group('Basic Coordinate Transformation', () {
      ...
      test('should handle negative pan offsets', () {
        ...
        expect(
            col, equals(2)); // (100 - (-20)) = 120, 120/60 = 2.0 -> floor = 2
      });
    });
    ...
    group('Bounds Validation', () {
      ...
      test('should handle edge cases in bounds validation', () {
        ...
        // When & Then
        expect(_isWithinBounds(-1, 0, rows, cols), isFalse,
            reason: 'Negative row');
        expect(_isWithinBounds(0, -1, rows, cols), isFalse,
            reason: 'Negative col');
        expect(_isWithinBounds(19, 19, rows, cols), isTrue,
            reason: 'Max valid indices');
        expect(_isWithinBounds(20, 19, rows, cols), isFalse,
            reason: 'Row out of bounds');
        expect(_isWithinBounds(19, 20, rows, cols), isFalse,
            reason: 'Col out of bounds');
        expect(_isWithinBounds(100, 100, rows, cols), isFalse,
            reason: 'Both way out of bounds');
      });
    });
    ...
    group('Coordinate Transformation Edge Cases', () {
      test('should handle zero scale gracefully', () {
        ...
        expect(() {
          (globalOffset.dy - panOffset.dy) / scale;
      ...
      test('should handle very small scale values', () {
        ...
        // Then
        expect(
            row, equals(16)); // (100/0.1) = 1000, 1000/60 = 16.66 -> floor = 16
        expect(
            col, equals(16)); // (100/0.1) = 1000, 1000/60 = 16.66 -> floor = 16
      });
      ...
      test('should handle floating point precision issues', () {
        ...
        expect(
            row,
            equals(
                4)); // 299.99999999999994 / 60 = 4.999999999999999 -> floor = 4
        expect(
            col,
            equals(
                3)); // 199.99999999999997 / 60 = 3.333333333333333 -> floor = 3
      });
    });
  });
}
