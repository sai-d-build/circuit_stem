...
void main() {
  group('Game Flow UI Tests', () {
    ...
    testWidgets('TC-L1-10 & TC-L1-27: Win condition shows win dialog/animation', (tester) async {
      // ARRANGE
      ...
      expect(find.byKey(const Key('win_dialog')), findsOneWidget, reason: 'Win dialog or animation should be displayed when all goals are met');
    });

    testWidgets('TC-L1-26: Goal tracker UI updates as goals are met', (tester) async {
      // ARRANGE
      final setup = await TestSetupHelper.pumpGameScreenForLevel(tester, 0);
      final container = setup.container;
      ...
      expect(find.textContaining('1/'), findsOneWidget, reason: 'Goal tracker UI should update when a goal is met');
    });
